...
#		executeString="insert into  blog_entries(href,title,text) values (\'%s\',\'%s\',\'%s\')" % (globalName[key]['href'],globalName[key]['title'],globalName[key]['content'].replace("'","\'"))
		...
		cur.execute(executeTemplate,(globalName[key]['href'],globalName[key]['title'],globalName[key]['content'].encode("utf-8"),globalName[key]['id'].split("-")[0],globalName[key]['des'].encode("utf-8"),globalName[key]['datetime']))
...
def upGlobal(globalName):
	...
	for key in globalName:
		...
		cur.execute(executeTemplate % (globalName[key]['href'].encode("utf-8"),globalName[key]['title'].encode("utf-8"),globalName[key]['content'].encode("utf-8"),globalName[key]['id'].split("-")[0].encode("utf-8"),globalName[key]['des'].encode("utf-8"),globalName[key]['datetime'].encode("utf-8"),globalName[key]['id'].split("-")[0].encode("utf-8")))
#		cur.execute(executeTemplate,("ggg","k","ggg","UUU原价","GGGGG"))
...
def update_db(con):
	return __query_db(con,"select oriId,datetime from blog.blog_entries")
